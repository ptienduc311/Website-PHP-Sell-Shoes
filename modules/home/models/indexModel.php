...
#-------------PRODUCT_CATEGORIES-----------------
//Lấy tên danh mục sản phẩm từ bảng product_categories
...
{
    $sql = "SELECT * FROM `products` WHERE `category_id` = $category_id  AND NOT product_status = 'inactive'";
...
//Lấy thông tin sản phẩm nổi bật
function get_featured_products()
{
    $sql = "SELECT * FROM `products` WHERE `is_featured` = 1  AND NOT product_status = 'inactive'";
    $data_product_featured = db_fetch_array($sql);

    //Xử lý cho chuyển trang chi tiết sản phẩm nổi bật
    $result = [];
    foreach ($data_product_featured as $item) {
        $item['url'] = "?mod=product&action=detail&id={$item['product_id']}";
        $result[] = $item;
    }
...
{
    $sql = "SELECT* FROM `sliders` ORDER BY display_order";
